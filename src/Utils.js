...
define([
  ...
  const Utils = {
     ...
     * @returns {Date} - Always return a Date object (now, if text was invalid)
     */
    parseOldDate: text => {
      let result = null;
      if (text) {
     ...
     */
    getXmlNodeText: node => {
      const result = Utils.parseXmlNode(node);
        ...
        keys.forEach(k => {
          ...
          if (typeof attr === 'object') {
            ...
            if (ko.length === 1 && ko[0] === 'text')
              ...
              obj[k] = Utils.reduceTextsToStrings(attr);
          }
        });
      }
      return obj;
